...
export default function CheckoutPage() {
  const { cartItems, getCartTotal, clearCart, getItemCount, isLoaded } = useCart(); // Added isLoaded
  ...
  useEffect(() => {
    if (isLoaded && itemCount === 0) {
      // This ensures redirection only happens client-side after cart state is confirmed
  ...
  }, [isLoaded, itemCount, router, toast]);


  if (isLoaded && itemCount === 0) {
    // This message is shown after client-side loading confirms cart is empty, while redirecting.
    // The initial server/client render would have been the form structure below.
    return <div className="text-center py-10">Your cart is empty. Redirecting to products page...</div>;
  }

  // If cart is not loaded yet (`!isLoaded`), or if it has items (`itemCount > 0`), render the form.
  // This ensures server render and initial client render (before `isLoaded` is true) match.
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {/* Render loading state or form based on isLoaded. For simplicity, we render the form.
          If !isLoaded, form fields could be disabled or show placeholders if desired, 
          but the key is the structure matches server.
      */}
      {!isLoaded && (
        <div className="text-center py-10 text-muted-foreground">Loading cart details...</div>
        // Or render the disabled form structure here to be extremely robust against hydration if needed.
        // For now, the logic ensures the path to the form below is taken by both server and initial client.
      )}
      ...
      {/* If !isLoaded and itemCount is 0, this will show the "Loading cart details..." above. 
          If isLoaded and itemCount is 0, it shows "Redirecting...".
          If isLoaded and itemCount > 0, it shows the form.
          This structure aims to keep server and initial client render consistent.
      */}
    </div>
  );
}
